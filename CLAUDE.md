...
**允许访问范围**：包 `Z*`, `Y*`, `$TMP`, `$ZADT_VSP`；类/程序/CDS/AMDP/接口均允许访问。

## 变更工作流

### 标准流程
1. 理解需求 → 分析相关对象 → **先给方案，不直接修改** → 确认方案后执行最小变更 → 生成测试点 → 人工复核后提交。

### 修改前必须输出
目标、影响对象、变更范围、风险点、回滚点、验证方法。

### 修改时原则
- 每次只做**一个清晰边界内**的变更。
- 不顺手重构无关代码，不随意扩大范围，不修改未被要求的对象。
- 优先最小变更，优先保持现有结构和命名。

### 修改后必须输出
修改内容、关键差异、测试建议、手工验证步骤、回滚建议。

### 高风险操作（必须暂停等待确认）
修改标准对象、大范围重构、改权限、改传输、改接口契约、改数据模型、改生产相关逻辑。

### Transport Request
- 所有可运输对象的修改前，必须确保已创建或绑定 Transport Request。
...
### 基础检查
- 代码是否能激活 / 语法错误 / dump 风险 / 命名规范

### 业务检查
- 业务规则正确 / 权限完整 / 边界条件 / 空值场景 / 无数据场景

### 技术检查
- 性能问题 / 减少数据库访问 / 避免过长循环 / 避免重复逻辑 / 事务一致性

### 回归检查
- 主流程 / 接口 / 报表 / 批处理 / 增强 是否正常

### 测试类型
- ABAP Unit / 手工执行 / 接口联调 / 批处理验证 / 日志与 dump 检查

## 项目特定规则（来自 ABAP_PROJECT_RULES.md）

- **先理解再修改**: 修改前务必理解已有代码
- **最小变更优先**: 优先选择最小改动方案
- **禁止编造 SAP 对象**: 不得虚构表名、函数模块或 BAPI
- **Clean Core**: 避免违反 SAP Clean Core 原则的改动
- **输出结构**: 结论 → 变更方案 → 代码 → 测试建议 → 风险 → 待确认问题
- **信息不足时先提问**: 不确定时明确标注，优先保守方案
